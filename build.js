...
snx.networks
	...
	.forEach(network => {
		...
		for (const { name, address } of Object.values(targets)) {
			redirects.push({
				source: `/${network !== 'mainnet' ? network + '/' : ''}${name}`,
				destination: `https://${network !== 'mainnet' ? network + '.' : ''}etherscan.io/address/${address}`,
			});
		}
	});
...
fs.writeFileSync(redirectsFile, JSON.stringify(finalJSON));
